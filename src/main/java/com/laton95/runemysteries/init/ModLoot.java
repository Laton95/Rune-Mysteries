...
public class ModLoot {
	...
	public static final ResourceLocation ESSENCE_MINE = LootTableList.register(new ResourceLocation(RuneMysteries.MOD_ID, "chests/essence_mine"));
	
	private static final String[] lootTables = {
			//			"inject/chests/abandoned_mineshaft",
			//			"inject/chests/buried_treasure",
			...
			//			"inject/chests/underwater_ruin_big",
			//			"inject/chests/underwater_ruin_small"
	...
	@SubscribeEvent
	public static void lootRegister(FMLCommonSetupEvent event) {
		ModLog.info("Registering loot tables");
		
		for(String path : lootTables) {
			LootTableList.register(new ResourceLocation(RuneMysteries.MOD_ID, path));
		}
	}
	
	@SubscribeEvent
	public static void lootLoad(LootTableLoadEvent event) {
		ModLog.info("Loading loot tables");
	...
	private static LootEntryTable getInjectEntry(String name, int weight) {
		return new LootEntryTable(new ResourceLocation(RuneMysteries.MOD_ID, "inject/" + name), weight, 0, new LootCondition[0], "runemysteries_inject_entry");
	}
}
