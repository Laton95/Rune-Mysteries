...
public class TeleportHelper
{
	...
	private static class PassengerHelper
	{
		 ...
		 *
		 * @param entity
		 *            The root entity. If you have multiple stacked entities
		 ...
		 *
		 * @param passenger
		 *            The passenger you are looking for.
			...
			int targetDim, double xCoord, double yCoord, double zCoord, float yaw, float pitch)
	{
		...
		Entity newEntity = EntityList.createEntityByIDFromName(new ResourceLocation(EntityList.getEntityString(entity)),
				targetWorld);
		if (newEntity != null)
		{
			/** copyDataFromOld */
			...
			int sourceDim, int targetDim, double xCoord, double yCoord, double zCoord, float yaw, float pitch)
	{
		...
		net.minecraftforge.fml.common.FMLCommonHandler.instance().firePlayerChangedDimensionEvent(player, sourceDim,
				targetDim);
		player.setLocationAndAngles(xCoord, yCoord, zCoord, yaw, pitch);
