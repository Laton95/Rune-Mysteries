...
public class ModCriterion implements ICriterionTrigger {
	...
	public ModCriterion(String parString) {
		super();
		ID = new ResourceLocation(RuneMysteries.MOD_ID, parString);
	}
	
	public ModCriterion(ResourceLocation parRL) {
		super();
		ID = parRL;
	}
	...
	public void addListener(PlayerAdvancements playerAdvancementsIn, ICriterionTrigger.Listener listener) {
		ModCriterion.Listeners tameanimaltrigger$listeners = (ModCriterion.Listeners) this.listeners.get(playerAdvancementsIn);
		
		if(tameanimaltrigger$listeners == null) {
			tameanimaltrigger$listeners = new ModCriterion.Listeners(playerAdvancementsIn);
	 ...
	 */
	@Override
	public ModCriterion.Instance deserializeInstance(JsonObject json, JsonDeserializationContext context) {
		return new ModCriterion.Instance(this.getId());
	}
