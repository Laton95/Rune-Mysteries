		...
		public Start(World worldIn, Random random, int chunkX, int chunkZ, Biome biomeIn)
		{
			...
			if (altar != null && !altar.isPlaced())
			{
				if (!altar.isBiomeDependant() || altar.isBiomeViable(biomeIn))
				{
					...
					ComponentSoulAltar componentRuneAltar = new ComponentSoulAltar(random, chunkX * 16 + random.nextInt(6) + 1, chunkZ * 16 + random.nextInt(6) + 1, altar.getName(), altar.getRoom(), depth);
