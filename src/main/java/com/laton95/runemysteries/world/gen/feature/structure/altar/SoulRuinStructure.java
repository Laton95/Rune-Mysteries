...
public class SoulRuinStructure extends Structure<NoFeatureConfig> {
	...
	public boolean hasStartAt(ChunkGenerator<?> chunkGen, Random rand, int chunkPosX, int chunkPosZ) {
		return RuneMysteries.ruinManager.isRuinInChunk(EnumRuneType.SOUL, chunkGen, new ChunkPos(chunkPosX, chunkPosZ));
	}
	...
	public BlockPos findNearest(World world, ChunkGenerator<? extends GenerationSettings> chunkGenerator, BlockPos pos, int radius, boolean p_211405_5_) {
		return RuneMysteries.ruinManager.getRuinPosition(EnumRuneType.SOUL, chunkGenerator);
	}
	...
	public class Start extends StructureStart {
		...
		public void init(ChunkGenerator<?> generator, TemplateManager templateManager, int chunkX, int chunkZ, Biome biome) {
			int i = chunkX * 16;
			int j = chunkZ * 16;
			BlockPos pos = new BlockPos(i, 90, j + 4);
			SoulRuinPieces.addPieces(templateManager, pos, this.components, rand);
