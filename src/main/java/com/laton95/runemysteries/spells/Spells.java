		...
		for (SpellBase spell : spellList)
		{
			...
			for (SpellCost spellCost : spell.getCosts())
			{
				for (ItemStack itemStack : items)
				{
					...
					{
						throw new IllegalArgumentException("Spell has multiple costs with the same item: "
								+ spell.getName());
					}
				}
			...
			{
				throw new IllegalArgumentException("Spell is uncastable, too many costs to fit in player inventory: "
						+ spell.getName());
			}
		}
	}
